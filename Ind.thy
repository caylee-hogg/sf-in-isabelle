...
(* CH: In Isabelle, we prove things that have type prop but I believe there
...
(* CH: otice the difference in types between the two statements:  again, I believe this 
...
(* WCH: Now, the above definition is essentially redundant in Isabelle because the "bool" is used for propositions.  *)
...
(* CH: Now, I'm adding the 'intro' attribute to these rules so that they'll
...
(* CH:  This is a cute opportunity to demonstrate a neat little tool that
...
(* CH: finally, let's do it by hand just by applying rules. *)
...
(* CH: I believe that, in Isabelle, you can only ever do induction after applying the appropriate intro rules if it's quantified in the object logic. *)
...
(* CH: In Coq you can apply a theorem to arguments to get a new theorem.  
...
(* CH:  In order to more directly invoke the induction rule we want,
...
(* CH: At this point let's just call the numeral rule a thing and be
       done with it *)
...
(* CH:  I've been using the '+' a few times now without explicitly stating what
...
(* CH:  Okay, so there's a couple of ways you can do something similar to generalize dependent in Coq.  The way that the Isabelle tutorial recommends is using
