...
*)
(* CH:  As far as I know, there's no equivalent in Isabelle to this kind of
        tactic structure *)
...
(* CH:  The last is, I think, the most idiomatic Isabelle auto way.  W00000!!! *)
...
(* CH: The equivalent of omega is arith *)
...
(* CH:  A lot of this lower level machinery on manipulating the proof state is done
   with Isar proofs.  *)
...
datatype id = Id nat
(* CH:  Kay, so we can define a new equality or we can just use the built in equality that comes from datatypes 
...
(* CH:  In order for auto to save the day in this proof, we need to add
...
(* CH:  Add a more manual proof of this lemma *)
