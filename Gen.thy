...
lemma "double n = double m \<Longrightarrow> n = m"
apply (induct n)
apply (cases m)
apply simp
apply simp
(* whoops, okay we have a false premise now because m stayed the same
...
(* So in Isabelle we perform this with the keyword arbitrary *)
...
(* CH: so we don't actually have the following problems, per se, in Isabelle.
   The equivalent would be if we dealt with a theorem statement of the form
   \<forall> n m. double n = double m \<longrightarrow> n = m
   and then introduced the n & m explicitly.

   In reality, while the object-level quantifiers are there it's more idiomatic
   to state the theorem as being about meta-logic level quantification instead
*)
...
lemma "(n :: nat) + n = m + m \<Longrightarrow> n = m"
apply (induct n arbitrary: m)
apply (case_tac m)
apply simp
apply simp
...
lemma [simp]: "length (l ++ a # l') = Suc (length (l ++ l'))"
apply (induct l arbitrary: l')
by simp+

lemma "length l = n \<Longrightarrow> length (l ++ l) = n + n"
apply (induct l arbitrary: n)
by simp+
(* CH: idiomatic proofs in Isabelle *)
