...
datatype natprod = Pair nat nat ("<_,_>" 1)
...
primrec fst' :: "natprod \<Rightarrow> nat" where
"fst' (Pair x y) = x"

primrec snd' :: "natprod \<Rightarrow> nat" where
"snd' (Pair x y) = y"
...
(* CH: okay, so now let's go ahead and switch over to using the built in isabelle
       product of naturals: nat \<times> nat *)

term fst
term snd

primrec swapPair :: "nat \<times> nat \<Rightarrow> nat \<times> nat" where
"swapPair (x, y) = (y, x)"
...
(* CH: well....there's a lot of simplification happening here so it follows automatically.
       Here, let's try our built in type to see if we can make this get a little more
       stuck *)
...
primrec repeat :: "nat \<Rightarrow> nat \<Rightarrow> nat list" where
"repeat n 0 = []" |
"repeat n (Suc c) = n # (repeat n c)"
...
primrec length' :: "nat list \<Rightarrow> nat" where
"length' [] = 0" |
...
primrec app :: "nat list \<Rightarrow> nat list \<Rightarrow> nat list" where
"app [] l2 = l2" |
"app (h # t) l2 = h # (app t l2)"

notation app (infixr "++" 50)
...
(* CH: we can use the hd & tl functions, which are defined in Isabelle almost identically
       to below *)
...
lemma "nonzeros [0,1,0,2,3,0,0] = [1,2,3]"
apply simp
done
