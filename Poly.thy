...
(* CH:  inversion in Coq is interesting!  It doesn't quite correspond to 
...
(* CH:  Yeah, since the = in Isabelle isn't an inductive type we can't
...
(* CH:  I find the above interesting, because the fact that
...
(* CH: Okay, the above proof was actually a little silly.  The real way
...
(* CH: So in order to do this proof without using auto we need a couple of cute tools that we haven't
...
(* CH:  Again, don't want to use auto here!  I might need to switch to Isar
