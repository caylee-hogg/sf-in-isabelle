...
(* CH:  In Isabelle, cases is the tactic to use if you are dealing with a schematic variable! *)
...
(* CH: cases is the equivalent of destruct *)

(* CH: what about a variable that is universally quantified in the meta-logic?  For that, 
       we need to use case_tac *)

lemma "\<And> n. (myplus n 1 = 0) = False"
apply (case_tac n)
by simp_all
(* CH: really, if you are using *_tac then there's something wrong - 
...
(* CH:  Okay, so here's our first Isar proof! It's a little
...
(* CH:  Okay, so we can do some of this stuff with Isar but in general Ltac doesn't have an equivalent in Isabelle *)
...
(* CH:  We'll do this one Isar style! *)
...
(* CH:  induction in Isabelle is with the induct rule *)
...
(* CH:  an Isar style proof where we only prove the interesting case in the Isar style and declare that the remaining 
goals should be solved by simp *)

(* CH:  Similar to cases, we use induct_tac when the variable is universally quantified - though we really shouldn't *)
