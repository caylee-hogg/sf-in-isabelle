...
primrec nextWeekday :: "day \<Rightarrow> day" where
...
(* as we can we have a theorem that is called nextWeekday.simps
   ...
   now, instead, let's use the fun facility to define this operation
 *)

fun nextWeekday' :: "day \<Rightarrow> day" where
"nextWeekday' Monday = Tuesday"|
"nextWeekday' Tuesday = Wednesday" |
"nextWeekday' Wednesday = Thursday" |
"nextWeekday' Thursday = Friday" |
...
(* now a difference between using definition in Coq and definition in isabelle 
   is that definition in Isabelle is, by default, opaque and cannot be reduced. 
 
   So if we want to use it the same way we need to do the following

*)

lemma "oddb (Suc 0) = True'"
...
proof (cases b)
  case True'
   show ?thesis using True' by assumption
 next
  case False'
   assume "andb b c = True'"
