theory MoreStlc
imports Stlc Relation Smallstep 
begin
...
datatype ty = TyArr ty ty | TyProd ty ty | TySum ty ty | TyList ty | TyNat
...
datatype tm = TmVar id | TmApp tm tm | TmAbs id ty tm | TmPair tm tm | TmFst tm
            | TmSnd tm | TmInl ty tm | TmInr ty tm | TmCase tm id tm id tm
            | TmNil | TmCons tm tm | TmLCase tm tm id id tm
            | TmNat nat | TmSucc tm | TmPred tm | TmMult tm tm | TmIf0 tm tm tm
            | TmLet id tm tm | TmFix tm
...
fun subst where
"subst x s (TmVar x') = (if x=x' then s else TmVar x')" |
"subst x s (TmAbs x' T t) = TmAbs x' T (if x=x' then t else (subst x s t))" |
"subst x s (TmApp t t') = TmApp (subst x s t) (subst x s t')" |
"subst x s (TmPair t t') = TmPair (subst x s t) (subst x s t')" |
"subst x s (TmFst t) = TmFst (subst x s t)" |
"subst x s (TmSnd t) = TmSnd (subst x s t)" |
"subst x s (TmInl ty t) = TmInl ty (subst x s t)" |
"subst x s (TmInr ty t) = TmInr ty (subst x s t)" |
"subst x s (TmCase tmc x' tml x'' tmr) = TmCase (subst x s tmc) x' (if x=x' then tml else (subst x s tml)) x'' (if x=x'' then tmr else (subst x s tmr))" |
"subst x s TmNil = TmNil" |
"subst x s (TmCons th tt) = TmCons (subst x s th) (subst x s tt)" |
"subst x s (TmLCase tml th x' x'' tt) = TmLCase (subst x s tml) (if x=x' then th else (subst x s th)) x' x'' (if x = x'' then tt else (subst x s tt))" |
"subst x s (TmNat n) = TmNat n" |
"subst x s (TmSucc t) = TmSucc (subst x s t)" |
"subst x s (TmPred t) = TmPred (subst x s t)" |
"subst x s (TmMult t t') = TmMult (subst x s t) (subst x s t')" |
"subst x s (TmIf0 t t' t'') = TmIf0 (subst x s t) (subst x s t') (subst x s t'')" |
"subst x s (TmLet x' t t') = TmLet x' (subst x s t) (if x = x' then t' else (subst x s t'))" |
